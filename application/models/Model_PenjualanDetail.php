...
class Model_PenjualanDetail extends CI_Model
{
	...
	var $column_order = array('pjd_id', 'mtl_nama', 'pjd_qty', 'smt_nama', 'pjd_harga', 'pjd_status'); //set column field database for datatable orderable
	...
	{
		$this->db->select("SUM(pjd_harga) as jumlah_bayar");
		$this->db->from($this->table);
		$this->db->where('pjd_pjl_id', $id);
		$query = $this->db->get();
	...
	{
		$this->db->select("COUNT(pjd_mtl_id) as total_item");
		$this->db->from($this->table);
		$this->db->where('pjd_pjl_id', $id);
		$query = $this->db->get();
