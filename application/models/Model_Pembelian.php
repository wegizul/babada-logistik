...
class Model_Pembelian extends CI_Model
{
	...
	public function pembelian_material($id)
	{
		$this->db->select("pbd_qty, pbl_id, pbl_tanggal, pbl_supplier, pbl_total_item, pbd_pbl_id, pbd_mtl_id, pbd_satuan, mtl_id, mtl_nama");
		$this->db->from($this->table);
		$this->db->join('pembelian_detail', 'pbd_pbl_id = pbl_id', 'left');
		$this->db->join('material', 'mtl_id = pbd_mtl_id', 'left');
		$this->db->where('pbd_mtl_id', $id);
		$this->db->order_by('pbl_tanggal', 'asc');
		$query = $this->db->get();
