			...
			<form role="form col-lg" name="TambahEdit" id="frm_material">
				<div class="modal-body form">
					<div class="row">
						...
						<p><b><sup class="text-danger">*</sup>Note : </b>Untuk menambahkan premix, silahkan menambahkan melalui menu Data Premix</p>
						<div class="col-lg-5">
							<div class="form-group">
						...
						<div class="col-lg-3">
							<div class="form-group">
								...
								<input type="number" min="0" class="form-control" name="mtl_barcode" id="mtl_barcode" required>
							</div>
						</div>
...
<div class="modal fade" id="modal_lihat_transaksi" role="dialog">
	<div class="modal-dialog modal-xl">
		<div class="modal-content">
			<div class="modal-header">
				<h6 class="modal-title"><i class="fas fa-cube"></i> Transaksi Pembelian & Penjualan Material </h6> <label id="stok" style="margin-left:55%; padding: 10px; background-color:goldenrod;"></label>
				<span type="button" aria-hidden="true" class="close" data-dismiss="modal" aria-label="Close" onclick="reset_form()">&times;</span>
			</div>
			<div class="modal-body">
				<div class="row">
					<label><i class="fas fa-circle-down"></i> Pembelian / Barang Masuk </label>&nbsp;<label id="pembelian" style="color: green;"></label>
					...
					<label><i class="fas fa-circle-up"></i> Penjualan / Barang Keluar </label>&nbsp;<label id="penjualan" style="color: green;"></label>
					<table class="table table-striped table-bordered">
						<thead>
							<tr>
...
<script>
	...
	function lihat_transaksi(id) {
		...
		$.ajax({
			type: "GET",
			url: "lihat_transaksi/" + id,
			dataType: "json",
			success: function(data) {
				var obj = Object.entries(data);
				var pbl = obj[0][1];
				var pjl = obj[1][1];
				var data_pbl = Object.entries(pbl);
				var data_pjl = Object.entries(pjl);

				data_pbl.forEach((db) => {
					$("#tabelPembelian").append(db[1]);
				});
				data_pjl.forEach((dj) => {
					$("#tabelPenjualan").append(dj[1]);
				});
				$("#stok").html('Sisa Stok ' + obj[2][1]);
				$("#pembelian").html(' (Total Pembelian ' + obj[3][1] + ')');
				$("#penjualan").html(' (Total Penjualan ' + obj[4][1] + ')');

				$('#modal_lihat_transaksi').modal({
					show: true,
					keyboard: false,
					backdrop: 'static'
				});
