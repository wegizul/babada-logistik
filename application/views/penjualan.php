						...
						<div class="col-lg-8" style="margin-top: 20px;">
							<select class="form-control select2" style="width: 100%;" onChange="cari_material(this.value)" autofocus>
								...
								<?php foreach ($material as $m) { ?>
									<!-- if ($m->mtl_stok > 0) { ?> -->
									<option value="<?= $m->mtl_id ?>"><?= $m->mtl_nama ?> (stok <?= $m->mtl_stok ?>)</option>
									<!--  } else { ?>
										<option value="<?= $m->mtl_id ?>" disabled><?= $m->mtl_nama ?> (stok habis)</option>
								 } ?> -->
								<?php } ?>
							</select>
						</div>
						...
						<div class="col-lg-12">
							<div class="form-group">
								...
								<select class="form-control" name="pjl_status2" id="pjl_status2">
									...
									<option value="6">Tolak Penjualan</option>
								</select>
							</div>
						</div>
					</div>
...
<div class="modal fade" id="modal_catatan" role="dialog">
	<div class="modal-dialog modal-md">
		<div class="modal-content">
			<div class="modal-header">
				<h6 class="modal-title"><i class="fas fa-comment"></i> Catatan Pengiriman</h6>
				<span type="button" aria-hidden="true" class="close" data-dismiss="modal" aria-label="Close" onclick="reset_form()">&times;</span>
			</div>
			<form role="form col-lg" name="TambahEdit" id="frm_catatan">
				<div class="modal-body form">
					<div class="row">
						<div class="col-lg-12">
							<div class="form-group">
								<textarea rows="3" class="form-control" name="pjl_catatan" id="pjl_catatan" disabled></textarea>
							</div>
						</div>
					</div>
				</div>
				<div class="modal-footer">
					<button type="button" id="close" class="btn btn-danger btn-sm" data-dismiss="modal" aria-label="Close" onclick="reset_form()"><i class="fas fa-times-circle"></i> Tutup</button>
				</div>
			</form>
		</div>
	</div>
...
<div class="modal fade" id="modal_edit_tgl" role="dialog">
	<div class="modal-dialog modal-md">
		<div class="modal-content">
			<div class="modal-header">
				<h6 class="modal-title"><i class="fas fa-calendar-alt"></i> Ubah Tanggal</h6>
				<span type="button" aria-hidden="true" class="close" data-dismiss="modal" aria-label="Close" onclick="reset_form()">&times;</span>
			</div>
			<form role="form col-lg" name="TambahEdit" id="frm_edit_tgl">
				<div class="modal-body form">
					<input type="hidden" name="pjl_id" id="pjl_id4" value="">
					<div class="row">
						<div class="col-lg-12">
							<div class="form-group">
								<input type="date" class="form-control" name="pjl_tanggal" id="pjl_tanggal2">
							</div>
						</div>
					</div>
				</div>
				<div class="modal-footer">
					<button type="submit" id="simpan_tgl" class="btn btn-dark btn-sm"><i class="fas fa-check-circle"></i> Simpan</button>
				</div>
			</form>
		</div>
	</div>
...
<script>
	...
	function tambah(val) {
		...
		$('#list_item').append('<tr id="row' + i + '">' +
			...
			<td width="30%">
				...
				<small id="er_qty" class="text-red"></small>
			</td>
			<td width="20%">
				` + val.smt_nama + `
	...
	function edit_tgl(pjl) {
		$("#pjl_id4").val(pjl);
		$("frm_edit_tgl").trigger("reset");
		$('#modal_edit_tgl').modal({
			show: true,
			keyboard: false,
			backdrop: 'static'
		});
	...
	$("#frm_edit_tgl").submit(function(e) {
		e.preventDefault();
		$("#simpan_tgl").html("Menyimpan...");
		$(".btn").attr("disabled", true);
		$.ajax({
			type: "POST",
			url: "simpan_edit_tgl",
			...
			success: function(d) {
				...
				if (res.status == 1) {
					toastr.success(res.desc);
					drawTable();
					$("#modal_edit_tgl").modal("hide");
				} else {
					toastr.error(res.desc);
				}
				$("#simpan_tgl").html("<i class='fas fa-check-circle'></i> Simpan");
				$(".btn").attr("disabled", false);
			},
			error: function(jqXHR, namaStatus, errorThrown) {
				$("#simpan_tgl").html("Error");
				$(".btn").attr("disabled", false);
				alert('Error get data from ajax');
			}
		});
