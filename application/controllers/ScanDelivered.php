...
class ScanDelivered extends CI_Controller
{
	...
	public function tampil()
	{
		...
		$this->load->view('scan_delivered', $data);
		$this->load->view('background_bawah');
	}

	public function ajax_list_tracking()
	{
		$list = $this->tracking->get_datatables(4);
		$data = array();
		$no = $_POST['start'];
		foreach ($list as $tracking) {
			...
			$ket = "Paket sudah diterima oleh yang bersangkutan";
			$row = array();
		...
		$output = array(
			"draw" => $_POST['draw'],
			"recordsTotal" => $this->tracking->count_all(4),
			"recordsFiltered" => $this->tracking->count_filtered(4),
			"data" => $data,
	...
	public function simpan()
	{
		...
		if ($insert) $this->booking->update("booking_detail", array('bd_kode' => $data['tr_bd_kode']), $where);
