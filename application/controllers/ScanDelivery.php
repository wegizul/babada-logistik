...
class ScanDelivery extends CI_Controller
{
	...
	public function tampil()
	{
		...
		$this->load->view('scan_delivery', $data);
		$this->load->view('background_bawah');
	}

	public function ajax_list_tracking()
	{
		$list = $this->tracking->get_datatables(3);
		$data = array();
		$no = $_POST['start'];
		foreach ($list as $tracking) {
			...
			$ket = "Paket sedang diantar ke alamat tujuan ";
			$row = array();
		...
		$output = array(
			"draw" => $_POST['draw'],
			"recordsTotal" => $this->tracking->count_all(3),
			"recordsFiltered" => $this->tracking->count_filtered(3),
			"data" => $data,
	...
	public function simpan()
	{
		...
		$data['tr_tujuan'] = "Alamat Penerima";
